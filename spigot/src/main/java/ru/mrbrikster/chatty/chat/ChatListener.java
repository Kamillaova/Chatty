...
public class ChatListener implements Listener, EventExecutor {
  ...
  private String handleModerationMethod(
    AsyncPlayerChatEvent event, Player player, String message,
    StringBuilder logPrefixBuilder, ModerationMethod method
  ) {
    if (method.isBlocked()) {
      message = method.getEditedMessage();
      if (method.isUseBlock()) {
        if (configuration.getNode("general.completely-cancel").getAsBoolean(false)) { event.setCancelled(true); } else {
          event.getRecipients().clear();
